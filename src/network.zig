...
pub const JsonNetwork = struct {
    ...
    pub inline fn createErrorResponse(error_message: []const u8, status_code: u16) ![]const u8 {
        const error_json = try std.fmt.allocPrint(std.heap.page_allocator, "{{\"error\":\"{s}\",\"status\":{d}}}", .{ error_message, status_code });
    ...
    pub inline fn createSuccessResponse(data: []const u8) ![]const u8 {
        const success_json = try std.fmt.allocPrint(std.heap.page_allocator, "{{\"success\":true,\"data\":{s}}}", .{data});
