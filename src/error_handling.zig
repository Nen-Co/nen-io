...
pub const JsonErrorHandler = struct {
    ...
    pub inline fn formatError(err: anyerror, line: u32, column: u32) ![]const u8 {
        return try std.fmt.allocPrint(std.heap.page_allocator, "JSON Error at line {d}, column {d}: {s}", .{ line, column, @errorName(err) });
    }
    ...
    pub inline fn getErrorSeverity(err: anyerror) ErrorSeverity {
        return switch (err) {
            error.FileNotFound, error.FileTooLarge, error.InvalidFormat => .warning,

            error.NestingTooDeep, error.UnterminatedString, error.UnexpectedClosingBrace, error.UnexpectedClosingBracket, error.UnmatchedBrackets => .@"error",

            error.TokenPoolExhausted, error.MemoryError, error.NetworkError => .critical,
